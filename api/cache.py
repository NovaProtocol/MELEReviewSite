"""Cache-Control policy, shared by the API and the web service. (api copy)

The two services are independent deployments with no shared package, so this
module is duplicated rather than imported across them. Keep the copies identical:
a divergence here means one service caches what the other does not.
"""
